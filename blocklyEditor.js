...
async function executeLoop() {
    let iterations = 0;
    const maxIterations = Math.round(${loopCount});

    while (!stopExecution) {
        if (${foreverChecked} || iterations < maxIterations) {
...
// Call the loop execution and reset
await executeLoop();
`;
...
javascriptGenerator.forBlock['actionBlock'] = function(block) {
    ...
    // Wrap the action in an async function
    const code = `if (!stopExecution) { await (async () => { await npc.blockAction("${action}", ${duration}); })(); }\n`;
    return code;
};
